...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  // Fork: measure from the DEPLOYED binary's commit (install-stamp.json), not
  // the live checkout HEAD — a worktree deployment leaves HEAD stale by design
  // (see resolveDeployedSha), which would fabricate a huge behind-count. The
  // cache keys on the same base so a redeployed bundle invalidates immediately.
  const baseSha = (await resolveDeployedSha(updateRoot)) || currentSha

  if (!force && cacheIsFresh(cached, { branch, currentSha: baseSha, now })) {
  ...
  const status = target.slug
    ? await checkUpdatesViaApi({ slug: target.slug, branch, currentSha: baseSha, updateRoot })
  ...
  writeUpdateCheckCache({ fetchedAt: now, currentSha: baseSha, branch, status: result })
