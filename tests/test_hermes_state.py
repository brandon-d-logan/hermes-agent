...
class TestMessageStorage:
    ...
    def test_platform_message_id_round_trips(self, db):
        ...
        assert conv[0]["message_id"] == "ext_123"

    def test_cannot_set_timestamp_in_get_messages_as_conversation(self, db):
        """Timestamp is a DB-internal field and must never appear in the
        conversation-format output. The get_messages() raw view is the
        correct path for consumers that need it."""
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="no timestamp here")
